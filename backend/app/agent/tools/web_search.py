...
基于 duckduckgo-search 库（免 API key），失败时优雅降级。

注意：
- DuckDuckGo 偶尔会限流（HTTP 202 / 403），所以对外只暴露 ok/error，
...
class WebSearchArgs(BaseModel):
    ...
    region: str = Field(default="cn-zh", description="区域：cn-zh / wt-wt 等")
...
class WebSearchTool(BaseTool):
    ...
    name = "web_search"
    description = (
        "在公开互联网上搜索信息（DuckDuckGo），返回相关网页的标题/URL/摘要。"
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            with DDGS() as ddgs:
                # DDGS.text() 返回 generator，限制 max_results
                for r in ddgs.text(
