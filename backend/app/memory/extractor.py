...
- 有 → 提取（如用户身份、持久习惯、通用知识纠正）
- 没有 → 不提取（如当前任务的具体操作、临时参数、一次性指令）

大多数普通对话不包含高价值信息，此时请返回空数组。宁可漏提也不要错提。
...
你需要关注的事实类型包括：
1. "env_constraint"（环境约束）：操作系统类型、开发语言版本、服务端口等稳定不变的环境信息
2. "preference"（用户偏好）：用户的个人信息、交互风格偏好、工具选择等跨会话持久的偏好
3. "knowledge"（长期知识）：用户分享或确认的业务事实、固定配置、项目约定等

⚠ 以下高价值信息必须优先抽取（importance ≥ 0.85）：
- 用户身份信息（名字、职业、年龄等）→ 归为 "preference"
- 明确指令（用户要求 AI 以后所有对话都如何行为）→ 归为 "preference"
- 纠正反馈（用户纠正了 AI 的错误认知）→ 归为 "knowledge"

核心判断原则：假设用户明天开一个全新的、完全不同话题的对话，这条信息对那个新对话还有指导意义吗？
- 有 → 提取（如用户身份、持久习惯、环境约束、通用知识）
- 没有 → 不提取（如当前任务的具体操作、临时参数、一次性指令）

如果这段对话没有包含任何有长期保存价值的事实，请返回空数组。宁可漏提也不要错提。
